/**
 * Builds the system prompt for the interview AI, injecting the guide,
 * coding schema, and current position in the question tree.
 */

export interface InterviewConfig {
  persona: { system_prompt: string; name: string };
  guide: {
    questions: Array<{
      id: string;
      topic: string;
    ...
    branching?: Array<{
      trigger: { after: string; if_themes: string[] };
      follow_up: { id: string; topic: string; text: string; probes?: string[] };
    }>;
    closing: string;
  };
  coding_schema?: {
...
export function buildSystemPrompt(
  ...
  for (let i = 0; i < config.guide.questions.length; i++) {
    ...
    if (q.probes?.length) {
      parts.push(`   Probes: ${q.probes.join(" / ")}`);
    }
  }

  // Pending branch
  if (state.pending_branch_id && config.guide.branching) {
    const branch = config.guide.branching.find(
      (b) => b.follow_up.id === state.pending_branch_id
    );
    if (branch) {
      parts.push(`\n**IMPORTANT: Before moving to the next main question, ask this follow-up:**`);
      parts.push(`"${branch.follow_up.text}"`);
      if (branch.follow_up.probes?.length) {
        parts.push(`Probes: ${branch.follow_up.probes.join(" / ")}`);
      }
    }
  }
...
/**
 * Builds the prompt for the second-pass theme coding call.
 */
